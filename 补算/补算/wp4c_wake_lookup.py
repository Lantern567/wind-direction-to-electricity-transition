...
各 64 台 IEA 10MW），全部经 FLORIS v4.6.6 实算（与论文主分析同配置）。
...
"每风速档一次 run、72 风向批量"组织（18 run / 情境-TI，~22.5 s）。

输出：补算/output/wp4c_wake_lookup.npz
...
N_TURB = 64
NPZ = os.path.join(OUT, 'wp4c_wake_lookup.npz')
