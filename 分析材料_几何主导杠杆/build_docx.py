...
FIGMAP = {
    ...
    "2.8": ("Fig8_orientation_tail.png",
            "图 8｜朝向价值的长尾与其地理。(a) 171 场按平均朝向收益排序：红=>5%（7场）、橙=2-5%（19场）、灰=<2%；中位+0.34%；(b) 机制：收益 vs 尾流池（r=+0.36），着色=WCI（r=+0.04）；窄玫瑰×大池交互组 2.4%（2.5×全样本）；(c) 东亚季风海岸聚集：湄公河三角洲（F57+18.2/F126/F159）、杭州湾口 F66、珠江口 F91；欧洲 F155/F157 见注；(d) 逐年稳健性：F57 跨 48→80 台扩建稳定 16-22%；F155 场均值被首年 2 台稀释（现配置 ~12%）。"),
