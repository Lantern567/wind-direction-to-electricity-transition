...
=============================================================================
"""
import os, sys, csv, math, time, tempfile, shutil
import numpy as np
...
def precompute_wake_table(turbine_coords_x, turbine_coords_y,
    ...
    # Precompute wake efficiency matrix — batch by WD sector
    # (72 sectors × 35 WS bins each = much more stable than 2520 at once)
    wake_efficiency = np.ones((n_wd, n_ws), dtype=np.float64)

    t0 = time.perf_counter()

    # Process each WD sector as a separate FLORIS run (35 WS bins per run)
    # This is more stable than passing all 2520 conditions at once
    for iwd, wd in enumerate(wd_sectors):
        ws_vals = [float(ws) for ws in ws_bins]
        ti_vals = [float(ti)] * n_ws
        wd_vals = [float(wd)] * n_ws

        fm.set(wind_directions=wd_vals, wind_speeds=ws_vals,
               turbulence_intensities=ti_vals)
        fm.run()

        farm_powers = fm.get_farm_power() / 1000.0  # W → kW

        for iws in range(n_ws):
            p_wake = farm_powers[iws]
            p_no_wake = p_no_wake_bin[iws]
            if p_no_wake > 0:
                wake_efficiency[iwd, iws] = max(0.0, min(1.0, p_wake / p_no_wake))
            else:
                wake_efficiency[iwd, iws] = 1.0  # zero wind → no wake loss
